...
public:
    ...
    // this function is required in BST class for function PrintDirectorlikesSpecificGenre() , it searches genre and keeps tracks of genre count
...
/*
	...
	T* InsertValue(T* value, T* ploc)
	{
		...
		if (balance > 1 && value < ploc->lchild->ID)
			return rightRotate(ploc);
		
		if (balance < -1 && value > ploc->rchild->ID)
		...
		// Right Left Case 
		if (balance < -1 && value < root->rchild->ID)
		{
...
*/
    ...
    //deletes all show details on the basis of title
    void DeleteTitle(string value)
    {
        Search(value);
        if (loc != NULL)
        {
            ...
            if (loc->lchild == NULL && loc->rchild == NULL)
            {
                if (ploc == NULL)
                    root = NULL;
                else if (ploc->lchild == loc)
                    ploc->lchild = NULL;
                else
                    ploc->rchild = NULL;
                delete loc;
            }
            //check if deleting a node with left branch
            ...
            //check if deleting a node with right branch
            else if (loc->lchild == NULL && loc->rchild != NULL)
            {
                if (ploc == NULL)
                    root = loc->rchild;
            ...
            //check if deleting a node with 2 children
            else
            {
                T* tploc = ploc;
                T* tloc = loc;
                ...
                loc->lchild = tloc->lchild;
                loc->rchild = tloc->rchild;
                if (tploc == NULL)
                    root = NULL;
                else if (tploc->rchild == tloc)
                    tploc->rchild = loc;
                else
                    tploc->lchild = loc;
                delete tloc;
...
public:
    ...
    //declaration of objects for all classes that are either in BST or Doubly LinkedList
    BST<ShowsNode> Shows1;
    BST<Director_Node> Director1;
    BST<Actor_Node> Actor1;
...
int main()
{
    ...
    do {
        ...
        case 17:
            ...
            cout << "\nEnter Title of the Movie/Tv Show to delete:: ";
            cin.ignore();
            getline(cin, title);
            TNMS.Shows1.DeleteTitle(title);
